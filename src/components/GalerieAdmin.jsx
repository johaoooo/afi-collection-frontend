...
function GalerieAdmin() {
  ...
  const handleSave = async () => {
    try {
      const token = localStorage.getItem('adminToken');
      if (selectedItem) {
        await axios.put(`/api/admin/gallery/${selectedItem.id}`, formData, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-3">
              ...
              {formData.type === 'video' ? (
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Ex: https://www.youtube.com/embed/dQw4w9WgXcQ</p>
                </div>
              ) : (
                <div>
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button onClick={handleSave} className="px-4 py-2 bg-afi-green text-white rounded flex items-center gap-2">
