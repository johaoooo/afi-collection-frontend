      ...
      <div className="w-full px-2 sm:px-4 pt-2">
        <div className="w-full rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl">
          <div className="relative min-h-[60vh] sm:min-h-[85vh] md:min-h-[80vh] flex items-center justify-center">
            ...
            <button onClick={() => setCurrentSlide((prev) => (prev + 1) % slides.length)} className="absolute right-1 sm:right-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white rounded-full w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 flex items-center justify-center shadow-lg transition-all z-10"><FontAwesomeIcon icon={faChevronRight} className="text-xs sm:text-sm md:text-base text-gray-800" /></button>
          </div>
        </div>
      </div>
      ...
      <AnimatedSection direction="up">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-8 sm:py-12 md:py-16">
          <div className="text-center mb-6 sm:mb-8 md:mb-12">
            <div className="flex items-center justify-center gap-1 sm:gap-2 mb-1 sm:mb-2"><FontAwesomeIcon icon={faStore} className="text-afi-green text-lg sm:text-xl" /><h2 className="font-serif text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-gray-800 dark:text-white">Nos <em className="text-afi-green not-italic">sous-marques</em></h2></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
            {brands.map((brand, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} whileHover={{ y: -10 }} className={`group relative bg-gradient-to-br ${brand.color} rounded-xl sm:rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all cursor-pointer`}>
                <Link to={brand.path}>
          ...
          <div className="relative">
            ...
            <div ref={scrollContainerRef} className="flex overflow-x-auto gap-3 sm:gap-4 md:gap-6 pb-4 sm:pb-6 scroll-smooth" style={{ scrollbarWidth: 'thin' }}>
              {videoTestimonials.map((video, index) => (
                <motion.div key={video.id} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.1 }} whileHover={{ y: -5 }} className="min-w-[250px] sm:min-w-[300px] md:min-w-[380px] bg-white dark:bg-afi-dark-card rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all flex-shrink-0">
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-8 sm:py-12 md:py-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
            {actualites.map((act, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} whileHover={{ y: -5 }} className="bg-white dark:bg-afi-dark-card rounded-xl p-4 sm:p-5 md:p-6 shadow-md hover:shadow-xl transition-all">
      ...
      <AnimatedSection direction="up">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 pb-8 sm:pb-12 md:pb-16">
          <motion.div whileHover={{ scale: 1.02 }} className="bg-gradient-to-r from-afi-red to-red-700 rounded-2xl sm:rounded-3xl py-8 sm:py-10 md:py-12 px-3 sm:px-4 text-center shadow-xl">
            <div className="max-w-2xl mx-auto"><div className="inline-flex items-center gap-2 bg-white/20 rounded-full px-2 sm:px-3 md:px-4 py-0.5 sm:py-1 mb-3 sm:mb-4"><FontAwesomeIcon icon={faHandHoldingHeart} className="text-white text-xs sm:text-sm" /><span className="text-white text-[10px] sm:text-sm">Soutenez l'artisanat béninois</span></div><div className="flex items-center justify-center gap-2 mb-2"><FontAwesomeIcon icon={faHeart} className="text-white text-xl sm:text-2xl" /><h3 className="font-serif text-xl sm:text-2xl md:text-3xl font-bold text-white">Faire un Don</h3></div><p className="text-white/90 mb-4 sm:mb-6 text-xs sm:text-sm md:text-base">Chaque contribution soutient la formation des femmes et jeunes artisans via CFP Dorcas</p><motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}><Link to="/don" className="inline-block bg-white text-afi-red px-6 sm:px-8 py-2 sm:py-3 font-serif font-bold hover:bg-gray-100 transition-all rounded-xl shadow-lg text-sm sm:text-base">Contribuer maintenant</Link></motion.div></div>
          </motion.div>
        </div>
      </AnimatedSection>
    </>
