    ...
    <>
          ...
          <div className="text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <div className="inline-flex items-center gap-2 bg-white/20 px-4 py-1.5 rounded-full mb-4">
                <FontAwesomeIcon icon={faBagShopping} className="text-sm text-white" />
                <span className="text-xs font-medium text-white">AFISAC</span>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-6">
          <div className="relative max-w-md mx-auto">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
              <FontAwesomeIcon icon={faSearch} className="text-gray-400 dark:text-gray-500 text-sm" />
            </div>
            <input
              type="text"
              value={searchTerm}
              ...
              className="w-full pl-11 pr-10 py-2.5 rounded-xl bg-white dark:bg-afi-dark-card text-gray-700 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-afi-green shadow-md border dark:border-afi-dark-border"
            />
            {searchTerm && (
              <button onClick={() => setSearchTerm('')} className="absolute inset-y-0 right-0 pr-4 flex items-center">
                <FontAwesomeIcon icon={faTimes} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 text-sm" />
              </button>
            )}
          </div>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8">
          <button onClick={() => setFilter('all')} className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${filter === 'all' ? 'bg-afi-green text-white shadow-md' : 'bg-white dark:bg-afi-dark-card text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-afi-dark-border hover:bg-afi-green/10'}`}>Tous</button>
          <button onClick={() => setFilter('sac')} className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${filter === 'sac' ? 'bg-afi-green text-white shadow-md' : 'bg-white dark:bg-afi-dark-card text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-afi-dark-border hover:bg-afi-green/10'}`}>Sacs Macramé</button>
          <button onClick={() => setFilter('chaussure')} className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${filter === 'chaussure' ? 'bg-afi-green text-white shadow-md' : 'bg-white dark:bg-afi-dark-card text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-afi-dark-border hover:bg-afi-green/10'}`}>Chaussures</button>
        </div>
        
        {filteredProducts.length === 0 ? (
          <div className="text-center py-16">
            <div className="w-16 h-16 mx-auto bg-gray-100 dark:bg-afi-dark-card rounded-full flex items-center justify-center mb-3">
              <FontAwesomeIcon icon={faBagShopping} className="text-2xl text-gray-400" />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProducts.map((product, index) => (
              <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="bg-white dark:bg-afi-dark-card rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all group border dark:border-afi-dark-border">
                ...
                <div className="p-5">
                  ...
                  <div className="flex justify-between items-center mb-4">
                    <span className="font-mono text-afi-green font-bold text-lg">{product.price.toLocaleString('fr-FR')} FCFA</span>
                    <div className="flex items-center gap-1 text-xs text-gray-400"><FontAwesomeIcon icon={faShoppingCart} className="text-afi-green" /> {product.popularity}</div>
                  </div>
